...
%     [dustLogB, dustEndIdx] = ismember(varEnd, dustTable{:,2});
    ...
    if isWindExist == 'Y'
        ...
        for iteration = 1:25     % As for the theta_v, we are using the lower surface temp
            if iteration == 1
                upper_height = 3.0;
                lower_height = 1.5;
                %             delta_theta_v = temperatureTable{:, 19} - temperatureTable{:, 18};
                delta_theta_v = windTable{:, 49} - windTable{:, 48};        % Now using virtual temperatures
                delta_z = upper_height - lower_height;
                %             theta_v = temperatureTable{:, 18};
                theta_v_low = windTable{:, 48} + 273;     % Now using virtual temperature
                theta_v_high = windTable{:, 49} + 273;
                delta_u = windTable{:, 4} - windTable{:, 3};
                delta_v = windTable{:, 19} - windTable{:, 18};
                u_star_low = (((windTable{:, 78}).*(windTable{:, 78})) + ((windTable{:, 93}).*(windTable{:, 93}))).^(0.25);
                u_star_high = (((windTable{:, 79}).*(windTable{:, 79})) + ((windTable{:, 94}).*(windTable{:, 94}))).^(0.25);
                w_theta_v_low = windTable{:, 63};
                w_theta_v_high = windTable{:, 64};
                
                % Supplement variables for write out
                u_lower = windTable{:, 3};
                u_upper = windTable{:, 4};
                v_lower = windTable{:, 18};
                v_upper = windTable{:, 19};
                u_w_prime_low = windTable{:, 78};
                u_w_prime_high = windTable{:, 79};
                v_w_prime_low = windTable{:, 93};
                v_w_prime_high = windTable{:, 94};
                ...
                for idx = 1:length(CDT_Time)
                    try
                        ...
                        R_Bulk_uconv_1_5_3(idx, 1) = R_Bulk_up(idx, 1) ./ R_Bulk_down(idx, 1);
                    catch
                    ...
                    try
                        ...
                        L_low_uconv_1_5(idx, 1) = L_up_low(idx, 1) ./ L_down_low(idx, 1);
                        Term_3_uconv_1_5(idx, 1) = lower_height ./ L_low_uconv_1_5(idx, 1);
                    catch
                    ...
                    try
                        ...
                        L_high_uconv_3(idx, 1) = L_up_high(idx, 1) ./ L_down_high(idx, 1);
                        Term_3_uconv_3(idx, 1) = upper_height ./ L_high_uconv_3(idx, 1);
                    catch
                ...
                % Form a csv dump
                tempT_uconv_bulk = table(CDT_Time, bar_fence, R_Bulk_uconv_1_5_3, L_low_uconv_1_5, L_high_uconv_3, Term_3_uconv_1_5, Term_3_uconv_3);
                table_name_uconv_bulk = strcat(targetDate, '_calculation_uconv_1.5-3.0.csv');
                ...
                table_name_uconv_dump = strcat(targetDate, '_dump_uconv_1.5-3.0.csv');
            ...
            elseif iteration == 2
                ...
                lower_height = 1.5;
                %             delta_theta_v = temperatureTable{:, 19} - temperatureTable{:, 18};
                delta_theta_v = windTable{:, 55} - windTable{:, 54};        % Now using virtual temperatures
                delta_z = upper_height - lower_height;
                %             theta_v = temperatureTable{:, 18};
                theta_v_low = windTable{:, 54} + 273;     % Now using virtual temperature
                theta_v_high = windTable{:, 55} + 273;
                delta_u = windTable{:, 10} - windTable{:, 9};
                delta_v = windTable{:, 25} - windTable{:, 24};
                u_star_low = (((windTable{:, 84}).*(windTable{:, 84})) + ((windTable{:, 99}).*(windTable{:, 99}))).^(0.25);
                u_star_high = (((windTable{:, 85}).*(windTable{:, 85})) + ((windTable{:, 100}).*(windTable{:, 100}))).^(0.25);
                w_theta_v_low = windTable{:, 69};
                w_theta_v_high = windTable{:, 70};
                ...
                for idx = 1:length(CDT_Time)
                    try
                        R_Bulk_up(idx, 1) = g .* delta_theta_v(idx, 1) * delta_z;
                        R_Bulk_down(idx, 1) = theta_v_low(idx, 1) .* ((delta_u(idx, 1).*delta_u(idx, 1)) + (delta_v(idx, 1).*delta_v(idx, 1)));
                        R_Bulk_lconv_1_5_3(idx, 1) = R_Bulk_up(idx, 1) ./ R_Bulk_down(idx, 1);
                    catch
                        errmsg('red','bulk Richardson number is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                    ...
                    try
                        L_up_low(idx, 1) = -theta_v_low(idx, 1) .* ((u_star_low(idx, 1)).^3);
                        L_down_low(idx, 1) = k * g .* w_theta_v_low(idx, 1);
                        L_lconv_1_5(idx, 1) = L_up_low(idx, 1) ./ L_down_low(idx, 1);
                        Term_3_lconv_1_5(idx, 1) = lower_height ./ L_lconv_1_5(idx, 1);
                        L_val_1_5 = L_lconv_1_5;
                    catch
                        errmsg('red','Obukhov length for lower surface is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                    ...
                    try
                        L_up_high(idx, 1) = -theta_v_high(idx, 1) .* ((u_star_high(idx, 1)).^3);
                        L_down_high(idx, 1) = k * g .* w_theta_v_high(idx, 1);
                        L_high_lconv_3(idx, 1) = L_up_high(idx, 1) ./ L_down_high(idx, 1);
                        Term_3_lconv_3(idx, 1) = upper_height ./ L_high_lconv_3(idx, 1);
                    catch
                        errmsg('red','Obukhov length for higher surface is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                ...
                % Form a csv dump
                tempT_lconv_bulk1 = table(CDT_Time, bar_fence, R_Bulk_lconv_1_5_3, L_lconv_1_5, L_high_lconv_3, Term_3_lconv_1_5, Term_3_lconv_3);
                table_name_lconv_bulk1 = strcat(targetDate, '_calculation_lconv_1.5-3.0.csv');
                writetable(tempT_lconv_bulk1, table_name_lconv_bulk1);
                
                % Dump for calculation
                tempT_uconv_dump = table(CDT_Time, bar_fence, theta_v_low, theta_v_high, w_theta_v_low, w_theta_v_high, u_lower, u_upper, v_lower, v_upper, u_w_prime_low, u_w_prime_high, v_w_prime_low, v_w_prime_high);
                table_name_uconv_dump = strcat(targetDate, '_dump_lconv_1.5-3.0.csv');
                writetable(tempT_uconv_dump, table_name_uconv_dump);
            ...
            elseif iteration == 3
                ...
                lower_height = 4.5;
                %             delta_theta_v = temperatureTable{:, 19} - temperatureTable{:, 18};
                delta_theta_v = windTable{:, 57} - windTable{:, 56};        % Now using virtual temperatures
                delta_z = upper_height - lower_height;
                %             theta_v = temperatureTable{:, 18};
                theta_v_low = windTable{:, 56} + 273;     % Now using virtual temperature
                theta_v_high = windTable{:, 57} + 273;
                delta_u = windTable{:, 12} - windTable{:, 11};
                delta_v = windTable{:, 27} - windTable{:, 26};
                u_star_low = (((windTable{:, 86}).*(windTable{:, 86})) + ((windTable{:, 101}).*(windTable{:, 101}))).^(0.25);
                u_star_high = (((windTable{:, 87}).*(windTable{:, 87})) + ((windTable{:, 102}).*(windTable{:, 102}))).^(0.25);
                w_theta_v_low = windTable{:, 71};
                w_theta_v_high = windTable{:, 72};
                ...
                % Form a csv dump
                tempT_lconv_bulk2 = table(CDT_Time, bar_fence, R_Bulk_lconv_4_5_6, L_lconv_4_5, L_lconv_6, Term_3_lconv_4_5, Term_3_lconv_6);
                table_name_lconv_bulk2 = strcat(targetDate, '_calculation_lconv_4.5-6.0.csv');
                writetable(tempT_lconv_bulk2, table_name_lconv_bulk2);
                
                % Dump for calculation
                tempT_uconv_dump = table(CDT_Time, bar_fence, theta_v_low, theta_v_high, w_theta_v_low, w_theta_v_high, u_lower, u_upper, v_lower, v_upper, u_w_prime_low, u_w_prime_high, v_w_prime_low, v_w_prime_high);
                table_name_uconv_dump = strcat(targetDate, '_dump_lconv_4.5-6.0.csv');
                writetable(tempT_uconv_dump, table_name_uconv_dump);
            ...
            elseif iteration == 11
                ...
                lower_height = 4.5;
                %             delta_theta_v = temperatureTable{:, 19} - temperatureTable{:, 18};
                delta_theta_v = windTable{:, 162} - windTable{:, 161};        % tc_high_tower - tc_low_tower
                delta_z = upper_height - lower_height;
                %             theta_v = temperatureTable{:, 18};
                theta_v_low = windTable{:, 161} + 273;           % tc_low_tower + 273
                theta_v_high = windTable{:, 162} + 273;          % tc_high_tower + 273
                delta_u = windTable{:, 117} - windTable{:, 116};    % u_high_tower - u_low_tower
                delta_v = windTable{:, 132} - windTable{:, 131};  % v_high_tower - v_low_height
                ...
                for idx = 1:length(CDT_Time)
                    try
                        R_Bulk_up(idx, 1) = g .* delta_theta_v(idx, 1) * delta_z;
                        R_Bulk_down(idx, 1) = theta_v_low(idx, 1) .* ((delta_u(idx, 1).*delta_u(idx, 1)) + (delta_v(idx, 1).*delta_v(idx, 1)));
                        R_Bulk_Rel_4_5_6(idx, 1) = R_Bulk_up(idx, 1) ./ R_Bulk_down(idx, 1);
                    catch
                        errmsg('red','bulk Richardson number is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                    ...
                    try
                        L_up_low(idx, 1) = -theta_v_low(idx, 1) .* ((u_star_low(idx, 1)).^3);
                        L_down_low(idx, 1) = k * g .* w_theta_v_low(idx, 1);
                        L_Rel_4_5(idx, 1) = L_up_low(idx, 1) ./ L_down_low(idx, 1);
                        Term_3_Rel_4_5(idx, 1) = lower_height ./ L_Rel_4_5(idx, 1);
                    catch
                        errmsg('red','Obukhov length for lower surface is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                    ...
                    try
                        L_up_high(idx, 1) = -theta_v_high(idx, 1) .* ((u_star_high(idx, 1)).^3);
                        L_down_high(idx, 1) = k * g .* w_theta_v_high(idx, 1);
                        L_Rel_6(idx, 1) = L_up_high(idx, 1) ./ L_down_high(idx, 1);
                        Term_3_Rel_6(idx, 1) = upper_height ./ L_Rel_6(idx, 1);
                    catch
                        errmsg('red','Obukhov length for higher surface is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                ...
                % Form a csv dump
                tempT_uconv_bulk = table(CDT_Time, bar_fence, R_Bulk_Rel_4_5_6, L_Rel_4_5, L_Rel_6, Term_3_Rel_4_5, Term_3_Rel_6);
                table_name_uconv_bulk = strcat(targetDate, '_calculation_rel_4.5-6.0.csv');
                writetable(tempT_uconv_bulk, table_name_uconv_bulk);
            ...
            elseif iteration == 13
                ...
                lower_height = 3.0;
                %             delta_theta_v = temperatureTable{:, 19} - temperatureTable{:, 18};
                delta_theta_v = windTable{:, 50} - windTable{:, 49};        % Now using virtual temperatures
                delta_z = upper_height - lower_height;
                %             theta_v = temperatureTable{:, 18};
                theta_v_low = windTable{:, 49} + 273;     % Now using virtual temperature
                theta_v_high = windTable{:, 50} + 273;
                delta_u = windTable{:, 5} - windTable{:, 4};
                delta_v = windTable{:, 20} - windTable{:, 19};
                u_star_low = (((windTable{:, 79}).*(windTable{:, 79})) + ((windTable{:, 94}).*(windTable{:, 94}))).^(0.25);
                u_star_high = (((windTable{:, 80}).*(windTable{:, 80})) + ((windTable{:, 95}).*(windTable{:, 95}))).^(0.25);
                w_theta_v_low = windTable{:, 64};
                w_theta_v_high = windTable{:, 65};
                
                % Supplement variables for write out
                u_lower = windTable{:, 4};
                u_upper = windTable{:, 5};
                v_lower = windTable{:, 19};
                v_upper = windTable{:, 20};
                u_w_prime_low = windTable{:, 79};
                u_w_prime_high = windTable{:, 80};
                v_w_prime_low = windTable{:, 94};
                ...
                for idx = 1:length(CDT_Time)
                    try
                        R_Bulk_up(idx, 1) = g .* delta_theta_v(idx, 1) * delta_z;
                        R_Bulk_down(idx, 1) = theta_v_low(idx, 1) .* ((delta_u(idx, 1).*delta_u(idx, 1)) + (delta_v(idx, 1).*delta_v(idx, 1)));
                        R_Bulk_uconv_3_4_5(idx, 1) = R_Bulk_up(idx, 1) ./ R_Bulk_down(idx, 1);
                    catch
                        errmsg('red','bulk Richardson number is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                    ...
                    try
                        L_up_low(idx, 1) = -theta_v_low(idx, 1) .* ((u_star_low(idx, 1)).^3);
                        L_down_low(idx, 1) = k * g .* w_theta_v_low(idx, 1);
                        L_low_uconv_3(idx, 1) = L_up_low(idx, 1) ./ L_down_low(idx, 1);
                        Term_3_uconv_3(idx, 1) = lower_height ./ L_low_uconv_3(idx, 1);
                    catch
                        errmsg('red','Obukhov length for lower surface is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                    ...
                    try
                        L_up_high(idx, 1) = -theta_v_high(idx, 1) .* ((u_star_high(idx, 1)).^3);
                        L_down_high(idx, 1) = k * g .* w_theta_v_high(idx, 1);
                        L_high_uconv_4_5(idx, 1) = L_up_high(idx, 1) ./ L_down_high(idx, 1);
                        Term_3_uconv_4_5(idx, 1) = upper_height ./ L_high_uconv_4_5(idx, 1);
                    catch
                        errmsg('red','Obukhov length for higher surface is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                ...
                % Form a csv dump
                tempT_uconv_bulk = table(CDT_Time, bar_fence, R_Bulk_uconv_3_4_5, L_low_uconv_3, L_high_uconv_4_5, Term_3_uconv_3, Term_3_uconv_4_5);
                table_name_uconv_bulk = strcat(targetDate, '_calculation_uconv_3.0-4.5.csv');
                writetable(tempT_uconv_bulk, table_name_uconv_bulk);
                
                % Dump for calculation
                tempT_uconv_dump = table(CDT_Time, bar_fence, theta_v_low, theta_v_high, w_theta_v_low, w_theta_v_high, u_lower, u_upper, v_lower, v_upper, u_w_prime_low, u_w_prime_high, v_w_prime_low, v_w_prime_high);
                table_name_uconv_dump = strcat(targetDate, '_dump_uconv_3.0-4.5.csv');
                writetable(tempT_uconv_dump, table_name_uconv_dump);
            ...
            elseif iteration == 14
                ...
                lower_height = 4.5;
                %             delta_theta_v = temperatureTable{:, 19} - temperatureTable{:, 18};
                delta_theta_v = windTable{:, 51} - windTable{:, 50};        % Now using virtual temperatures
                delta_z = upper_height - lower_height;
                %             theta_v = temperatureTable{:, 18};
                theta_v_low = windTable{:, 50} + 273;     % Now using virtual temperature
                theta_v_high = windTable{:, 51} + 273;
                delta_u = windTable{:, 6} - windTable{:, 5};
                delta_v = windTable{:, 21} - windTable{:, 20};
                u_star_low = (((windTable{:, 80}).*(windTable{:, 80})) + ((windTable{:, 95}).*(windTable{:, 95}))).^(0.25);
                u_star_high = (((windTable{:, 81}).*(windTable{:, 81})) + ((windTable{:, 96}).*(windTable{:, 96}))).^(0.25);
                w_theta_v_low = windTable{:, 65};
                w_theta_v_high = windTable{:, 66};
                
                % Supplement variables for write out
                u_lower = windTable{:, 5};
                u_upper = windTable{:, 6};
                v_lower = windTable{:, 20};
                v_upper = windTable{:, 21};
                u_w_prime_low = windTable{:, 80};
                u_w_prime_high = windTable{:, 81};
                v_w_prime_low = windTable{:, 95};
                v_w_prime_high = windTable{:, 96};
                ...
                for idx = 1:length(CDT_Time)
                    try
                        R_Bulk_up(idx, 1) = g .* delta_theta_v(idx, 1) * delta_z;
                        R_Bulk_down(idx, 1) = theta_v_low(idx, 1) .* ((delta_u(idx, 1).*delta_u(idx, 1)) + (delta_v(idx, 1).*delta_v(idx, 1)));
                        R_Bulk_uconv_4_5_6(idx, 1) = R_Bulk_up(idx, 1) ./ R_Bulk_down(idx, 1);
                    catch
                        errmsg('red','bulk Richardson number is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                    ...
                    try
                        L_up_low(idx, 1) = -theta_v_low(idx, 1) .* ((u_star_low(idx, 1)).^3);
                        L_down_low(idx, 1) = k * g .* w_theta_v_low(idx, 1);
                        L_low_uconv_4_5(idx, 1) = L_up_low(idx, 1) ./ L_down_low(idx, 1);
                        Term_3_uconv_4_5(idx, 1) = lower_height ./ L_low_uconv_4_5(idx, 1);
                    catch
                        errmsg('red','Obukhov length for lower surface is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                    ...
                    try
                        L_up_high(idx, 1) = -theta_v_high(idx, 1) .* ((u_star_high(idx, 1)).^3);
                        L_down_high(idx, 1) = k * g .* w_theta_v_high(idx, 1);
                        L_high_uconv_6(idx, 1) = L_up_high(idx, 1) ./ L_down_high(idx, 1);
                        Term_3_uconv_6(idx, 1) = upper_height ./ L_high_uconv_6(idx, 1);
                    catch
                        errmsg('red','Obukhov length for higher surface is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                ...
                % Form a csv dump
                tempT_uconv_bulk = table(CDT_Time, bar_fence, R_Bulk_uconv_4_5_6, L_low_uconv_4_5, L_high_uconv_6, Term_3_uconv_4_5, Term_3_uconv_6);
                table_name_uconv_bulk = strcat(targetDate, '_calculation_uconv_4.5-6.0.csv');
                writetable(tempT_uconv_bulk, table_name_uconv_bulk);
                
                % Dump for calculation
                tempT_uconv_dump = table(CDT_Time, bar_fence, theta_v_low, theta_v_high, w_theta_v_low, w_theta_v_high, u_lower, u_upper, v_lower, v_upper, u_w_prime_low, u_w_prime_high, v_w_prime_low, v_w_prime_high);
                table_name_uconv_dump = strcat(targetDate, '_dump_uconv_4.5-6.0.csv');
                writetable(tempT_uconv_dump, table_name_uconv_dump);
            ...
            elseif iteration == 15
                ...
                lower_height = 6.0;
                %             delta_theta_v = temperatureTable{:, 19} - temperatureTable{:, 18};
                delta_theta_v = windTable{:, 52} - windTable{:, 51};        % Now using virtual temperatures
                delta_z = upper_height - lower_height;
                %             theta_v = temperatureTable{:, 18};
                theta_v_low = windTable{:, 51} + 273;     % Now using virtual temperature
                theta_v_high = windTable{:, 52} + 273;
                delta_u = windTable{:, 7} - windTable{:, 6};
                delta_v = windTable{:, 22} - windTable{:, 21};
                u_star_low = (((windTable{:, 81}).*(windTable{:, 81})) + ((windTable{:, 96}).*(windTable{:, 96}))).^(0.25);
                u_star_high = (((windTable{:, 82}).*(windTable{:, 82})) + ((windTable{:, 97}).*(windTable{:, 97}))).^(0.25);
                w_theta_v_low = windTable{:, 66};
                w_theta_v_high = windTable{:, 67};
                
                % Supplement variables for write out
                u_lower = windTable{:, 6};
                u_upper = windTable{:, 7};
                v_lower = windTable{:, 21};
                v_upper = windTable{:, 22};
                u_w_prime_low = windTable{:, 81};
                u_w_prime_high = windTable{:, 82};
                v_w_prime_low = windTable{:, 96};
                v_w_prime_high = windTable{:, 97};
                ...
                for idx = 1:length(CDT_Time)
                    ...
                    try
                        L_up_low(idx, 1) = -theta_v_low(idx, 1) .* ((u_star_low(idx, 1)).^3);
                        L_down_low(idx, 1) = k * g .* w_theta_v_low(idx, 1);
                        L_low_uconv_6(idx, 1) = L_up_low(idx, 1) ./ L_down_low(idx, 1);
                        Term_3_uconv_6(idx, 1) = lower_height ./ L_low_uconv_6(idx, 1);
                    catch
                        errmsg('red','Obukhov length for lower surface is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                    ...
                    try
                        L_up_high(idx, 1) = -theta_v_high(idx, 1) .* ((u_star_high(idx, 1)).^3);
                        L_down_high(idx, 1) = k * g .* w_theta_v_high(idx, 1);
                        L_high_uconv_10(idx, 1) = L_up_high(idx, 1) ./ L_down_high(idx, 1);
                        Term_3_uconv_10(idx, 1) = upper_height ./ L_high_uconv_10(idx, 1);
                    catch
                        errmsg('red','Obukhov length for higher surface is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                ...
                % Form a csv dump
                tempT_uconv_bulk = table(CDT_Time, bar_fence, R_Bulk_uconv_6_10, L_low_uconv_6, L_high_uconv_10, Term_3_uconv_6, Term_3_uconv_10);
                table_name_uconv_bulk = strcat(targetDate, '_calculation_uconv_6.0-10.0.csv');
                writetable(tempT_uconv_bulk, table_name_uconv_bulk);
                
                % Dump for calculation
                tempT_uconv_dump = table(CDT_Time, bar_fence, theta_v_low, theta_v_high, w_theta_v_low, w_theta_v_high, u_lower, u_upper, v_lower, v_upper, u_w_prime_low, u_w_prime_high, v_w_prime_low, v_w_prime_high);
                table_name_uconv_dump = strcat(targetDate, '_dump_uconv_6.0-10.0.csv');
                writetable(tempT_uconv_dump, table_name_uconv_dump);
            ...
            elseif iteration == 16
                ...
                lower_height = 3.0;
                %             delta_theta_v = temperatureTable{:, 19} - temperatureTable{:, 18};
                delta_theta_v = windTable{:, 56} - windTable{:, 55};        % Now using virtual temperatures
                delta_z = upper_height - lower_height;
                %             theta_v = temperatureTable{:, 18};
                theta_v_low = windTable{:, 55} + 273;     % Now using virtual temperature
                theta_v_high = windTable{:, 56} + 273;
                delta_u = windTable{:, 11} - windTable{:, 10};
                delta_v = windTable{:, 26} - windTable{:, 25};
                u_star_low = (((windTable{:, 85}).*(windTable{:, 85})) + ((windTable{:, 100}).*(windTable{:, 100}))).^(0.25);
                u_star_high = (((windTable{:, 86}).*(windTable{:, 86})) + ((windTable{:, 101}).*(windTable{:, 101}))).^(0.25);
                w_theta_v_low = windTable{:, 70};
                ...
                for idx = 1:length(CDT_Time)
                    try
                        R_Bulk_up(idx, 1) = g .* delta_theta_v(idx, 1) * delta_z;
                        R_Bulk_down(idx, 1) = theta_v_low(idx, 1) .* ((delta_u(idx, 1).*delta_u(idx, 1)) + (delta_v(idx, 1).*delta_v(idx, 1)));
                        R_Bulk_lconv_3_4_5(idx, 1) = R_Bulk_up(idx, 1) ./ R_Bulk_down(idx, 1);
                    catch
                        errmsg('red','bulk Richardson number is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                    ...
                    try
                        L_up_low(idx, 1) = -theta_v_low(idx, 1) .* ((u_star_low(idx, 1)).^3);
                        L_down_low(idx, 1) = k * g .* w_theta_v_low(idx, 1);
                        L_lconv_3(idx, 1) = L_up_low(idx, 1) ./ L_down_low(idx, 1);
                        Term_3_lconv_3(idx, 1) = lower_height ./ L_lconv_3(idx, 1);
                        L_val_3 = L_lconv_3;
                    catch
                        errmsg('red','Obukhov length for lower surface is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    ...
                    try
                        L_up_high(idx, 1) = -theta_v_high(idx, 1) .* ((u_star_high(idx, 1)).^3);
                        L_down_high(idx, 1) = k * g .* w_theta_v_high(idx, 1);
                        L_high_lconv_4_5(idx, 1) = L_up_high(idx, 1) ./ L_down_high(idx, 1);
                        Term_3_lconv_4_5(idx, 1) = upper_height ./ L_high_lconv_4_5(idx, 1);
                    catch
                        errmsg('red','Obukhov length for higher surface is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                ...
                % Form a csv dump
                tempT_lconv_bulk1 = table(CDT_Time, bar_fence, R_Bulk_lconv_3_4_5, L_lconv_3, L_high_lconv_4_5, Term_3_lconv_3, Term_3_lconv_4_5);
                table_name_lconv_bulk1 = strcat(targetDate, '_calculation_lconv_3.0-4.5.csv');
                writetable(tempT_lconv_bulk1, table_name_lconv_bulk1);
                
                % Dump for calculation
                tempT_uconv_dump = table(CDT_Time, bar_fence, theta_v_low, theta_v_high, w_theta_v_low, w_theta_v_high, u_lower, u_upper, v_lower, v_upper, u_w_prime_low, u_w_prime_high, v_w_prime_low, v_w_prime_high);
                table_name_uconv_dump = strcat(targetDate, '_dump_lconv_3.0-4.5.csv');
                writetable(tempT_uconv_dump, table_name_uconv_dump);
            ...
            elseif iteration == 17
                ...
                lower_height = 6.0;
                %             delta_theta_v = temperatureTable{:, 19} - temperatureTable{:, 18};
                delta_theta_v = windTable{:, 58} - windTable{:, 57};        % Now using virtual temperatures
                delta_z = upper_height - lower_height;
                %             theta_v = temperatureTable{:, 18};
                theta_v_low = windTable{:, 57} + 273;     % Now using virtual temperature
                theta_v_high = windTable{:, 58} + 273;
                delta_u = windTable{:, 13} - windTable{:, 12};
                delta_v = windTable{:, 28} - windTable{:, 27};
                u_star_low = (((windTable{:, 87}).*(windTable{:, 87})) + ((windTable{:, 102}).*(windTable{:, 102}))).^(0.25);
                u_star_high = (((windTable{:, 88}).*(windTable{:, 88})) + ((windTable{:, 103}).*(windTable{:, 103}))).^(0.25);
                w_theta_v_low = windTable{:, 72};
                ...
                for idx = 1:length(CDT_Time)
                    try
                        R_Bulk_up(idx, 1) = g .* delta_theta_v(idx, 1) * delta_z;
                        R_Bulk_down(idx, 1) = theta_v_low(idx, 1) .* ((delta_u(idx, 1).*delta_u(idx, 1)) + (delta_v(idx, 1).*delta_v(idx, 1)));
                        R_Bulk_lconv_6_8_5(idx, 1) = R_Bulk_up(idx, 1) ./ R_Bulk_down(idx, 1);
                    catch
                        errmsg('red','bulk Richardson number is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                    ...
                    try
                        L_up_low(idx, 1) = -theta_v_low(idx, 1) .* ((u_star_low(idx, 1)).^3);
                        L_down_low(idx, 1) = k * g .* w_theta_v_low(idx, 1);
                        L_lconv_6(idx, 1) = L_up_low(idx, 1) ./ L_down_low(idx, 1);
                        Term_3_lconv_6(idx, 1) = lower_height ./ L_lconv_6(idx, 1);
                    catch
                        errmsg('red','Obukhov length for lower surface is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                    ...
                    try
                        L_up_high(idx, 1) = -theta_v_high(idx, 1) .* ((u_star_high(idx, 1)).^3);
                        L_down_high(idx, 1) = k * g .* w_theta_v_high(idx, 1);
                        L_lconv_8_5(idx, 1) = L_up_high(idx, 1) ./ L_down_high(idx, 1);
                        Term_3_lconv_8_5(idx, 1) = upper_height ./ L_lconv_8_5(idx, 1);
                    catch
                        errmsg('red','Obukhov length for higher surface is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                ...
                % Form a csv dump
                tempT_lconv_bulk2 = table(CDT_Time, bar_fence, R_Bulk_lconv_6_8_5, L_lconv_6, L_lconv_8_5, Term_3_lconv_6, Term_3_lconv_8_5);
                table_name_lconv_bulk2 = strcat(targetDate, '_calculation_lconv_6.0-8.5.csv');
                writetable(tempT_lconv_bulk2, table_name_lconv_bulk2);
                
                % Dump for calculation
                tempT_uconv_dump = table(CDT_Time, bar_fence, theta_v_low, theta_v_high, w_theta_v_low, w_theta_v_high, u_lower, u_upper, v_lower, v_upper, u_w_prime_low, u_w_prime_high, v_w_prime_low, v_w_prime_high);
                table_name_uconv_dump = strcat(targetDate, '_dump_lconv_6.0-8.5.csv');
                writetable(tempT_uconv_dump, table_name_uconv_dump);
            ...
            elseif iteration == 18
                ...
                lower_height = 8.5;
                %             delta_theta_v = temperatureTable{:, 19} - temperatureTable{:, 18};
                delta_theta_v = windTable{:, 59} - windTable{:, 58};        % Now using virtual temperatures
                delta_z = upper_height - lower_height;
                %             theta_v = temperatureTable{:, 18};
                theta_v_low = windTable{:, 58} + 273;     % Now using virtual temperature
                theta_v_high = windTable{:, 59} + 273;
                delta_u = windTable{:, 14} - windTable{:, 13};
                delta_v = windTable{:, 29} - windTable{:, 28};
                u_star_low = (((windTable{:, 88}).*(windTable{:, 88})) + ((windTable{:, 103}).*(windTable{:, 103}))).^(0.25);
                u_star_high = (((windTable{:, 89}).*(windTable{:, 89})) + ((windTable{:, 104}).*(windTable{:, 104}))).^(0.25);
                w_theta_v_low = windTable{:, 73};
                w_theta_v_high = windTable{:, 74};
                ...
                for idx = 1:length(CDT_Time)
                    try
                        R_Bulk_up(idx, 1) = g .* delta_theta_v(idx, 1) * delta_z;
                        R_Bulk_down(idx, 1) = theta_v_low(idx, 1) .* ((delta_u(idx, 1).*delta_u(idx, 1)) + (delta_v(idx, 1).*delta_v(idx, 1)));
                        R_Bulk_lconv_8_5_10(idx, 1) = R_Bulk_up(idx, 1) ./ R_Bulk_down(idx, 1);
                    catch
                        errmsg('red','bulk Richardson number is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                    ...
                    try
                        L_up_low(idx, 1) = -theta_v_low(idx, 1) .* ((u_star_low(idx, 1)).^3);
                        L_down_low(idx, 1) = k * g .* w_theta_v_low(idx, 1);
                        L_lconv_8_5(idx, 1) = L_up_low(idx, 1) ./ L_down_low(idx, 1);
                        Term_3_lconv_8_5(idx, 1) = lower_height ./ L_lconv_8_5(idx, 1);
                    catch
                        errmsg('red','Obukhov length for lower surface is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                    ...
                    try
                        L_up_high(idx, 1) = -theta_v_high(idx, 1) .* ((u_star_high(idx, 1)).^3);
                        L_down_high(idx, 1) = k * g .* w_theta_v_high(idx, 1);
                        L_lconv_10(idx, 1) = L_up_high(idx, 1) ./ L_down_high(idx, 1);
                        Term_3_lconv_10(idx, 1) = upper_height ./ L_lconv_10(idx, 1);
                    catch
                        errmsg('red','Obukhov length for higher surface is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                ...
                % Form a csv dump
                tempT_lconv_bulk2 = table(CDT_Time, bar_fence, R_Bulk_lconv_8_5_10, L_lconv_8_5, L_lconv_10, Term_3_lconv_8_5, Term_3_lconv_10);
                table_name_lconv_bulk2 = strcat(targetDate, '_calculation_lconv_8.5-10.0.csv');
                writetable(tempT_lconv_bulk2, table_name_lconv_bulk2);
                
                % Dump for calculation
                tempT_uconv_dump = table(CDT_Time, bar_fence, theta_v_low, theta_v_high, w_theta_v_low, w_theta_v_high, u_lower, u_upper, v_lower, v_upper, u_w_prime_low, u_w_prime_high, v_w_prime_low, v_w_prime_high);
                table_name_uconv_dump = strcat(targetDate, '_dump_lconv_8.5-10.0.csv');
                writetable(tempT_uconv_dump, table_name_uconv_dump);
            ...
            elseif iteration == 19
                ...
                lower_height = 10.0;
                %             delta_theta_v = temperatureTable{:, 19} - temperatureTable{:, 18};
                delta_theta_v = windTable{:, 60} - windTable{:, 59};        % Now using virtual temperatures
                delta_z = upper_height - lower_height;
                %             theta_v = temperatureTable{:, 18};
                theta_v_low = windTable{:, 59} + 273;     % Now using virtual temperature
                theta_v_high = windTable{:, 60} + 273;
                delta_u = windTable{:, 15} - windTable{:, 14};
                delta_v = windTable{:, 30} - windTable{:, 29};
                u_star_low = (((windTable{:, 89}).*(windTable{:, 89})) + ((windTable{:, 104}).*(windTable{:, 104}))).^(0.25);
                u_star_high = (((windTable{:, 90}).*(windTable{:, 90})) + ((windTable{:, 105}).*(windTable{:, 105}))).^(0.25);
                w_theta_v_low = windTable{:, 74};
                w_theta_v_high = windTable{:, 75};
                ...
                for idx = 1:length(CDT_Time)
                    try
                        R_Bulk_up(idx, 1) = g .* delta_theta_v(idx, 1) * delta_z;
                        R_Bulk_down(idx, 1) = theta_v_low(idx, 1) .* ((delta_u(idx, 1).*delta_u(idx, 1)) + (delta_v(idx, 1).*delta_v(idx, 1)));
                        R_Bulk_lconv_10_15(idx, 1) = R_Bulk_up(idx, 1) ./ R_Bulk_down(idx, 1);
                    catch
                        errmsg('red','bulk Richardson number is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                    ...
                    try
                        L_up_low(idx, 1) = -theta_v_low(idx, 1) .* ((u_star_low(idx, 1)).^3);
                        L_down_low(idx, 1) = k * g .* w_theta_v_low(idx, 1);
                        L_lconv_10(idx, 1) = L_up_low(idx, 1) ./ L_down_low(idx, 1);
                        Term_3_lconv_10(idx, 1) = lower_height ./ L_lconv_10(idx, 1);
                    catch
                        errmsg('red','Obukhov length for lower surface is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                    ...
                    try
                        L_up_high(idx, 1) = -theta_v_high(idx, 1) .* ((u_star_high(idx, 1)).^3);
                        L_down_high(idx, 1) = k * g .* w_theta_v_high(idx, 1);
                        L_lconv_15(idx, 1) = L_up_high(idx, 1) ./ L_down_high(idx, 1);
                        Term_3_lconv_15(idx, 1) = upper_height ./ L_lconv_15(idx, 1);
                    catch
                        errmsg('red','Obukhov length for higher surface is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                ...
                % Form a csv dump
                tempT_lconv_bulk2 = table(CDT_Time, bar_fence, R_Bulk_lconv_10_15, L_lconv_10, L_lconv_15, Term_3_lconv_10, Term_3_lconv_15);
                table_name_lconv_bulk2 = strcat(targetDate, '_calculation_lconv_10.0-15.0.csv');
                writetable(tempT_lconv_bulk2, table_name_lconv_bulk2);
                
                % Dump for calculation
                tempT_uconv_dump = table(CDT_Time, bar_fence, theta_v_low, theta_v_high, w_theta_v_low, w_theta_v_high, u_lower, u_upper, v_lower, v_upper, u_w_prime_low, u_w_prime_high, v_w_prime_low, v_w_prime_high);
                table_name_uconv_dump = strcat(targetDate, '_dump_lconv_10.0-15.0.csv');
                writetable(tempT_uconv_dump, table_name_uconv_dump);
            ...
            elseif iteration == 20
                ...
                lower_height = 15.0;
                %             delta_theta_v = temperatureTable{:, 19} - temperatureTable{:, 18};
                delta_theta_v = windTable{:, 61} - windTable{:, 60};        % Now using virtual temperatures
                delta_z = upper_height - lower_height;
                %             theta_v = temperatureTable{:, 18};
                theta_v_low = windTable{:, 60} + 273;     % Now using virtual temperature
                theta_v_high = windTable{:, 61} + 273;
                delta_u = windTable{:, 16} - windTable{:, 15};
                delta_v = windTable{:, 31} - windTable{:, 30};
                u_star_low = (((windTable{:, 90}).*(windTable{:, 90})) + ((windTable{:, 105}).*(windTable{:, 105}))).^(0.25);
                u_star_high = (((windTable{:, 91}).*(windTable{:, 91})) + ((windTable{:, 106}).*(windTable{:, 106}))).^(0.25);
                w_theta_v_low = windTable{:, 75};
                w_theta_v_high = windTable{:, 76};
                ...
                for idx = 1:length(CDT_Time)
                    try
                        R_Bulk_up(idx, 1) = g .* delta_theta_v(idx, 1) * delta_z;
                        R_Bulk_down(idx, 1) = theta_v_low(idx, 1) .* ((delta_u(idx, 1).*delta_u(idx, 1)) + (delta_v(idx, 1).*delta_v(idx, 1)));
                        R_Bulk_lconv_15_20(idx, 1) = R_Bulk_up(idx, 1) ./ R_Bulk_down(idx, 1);
                    catch
                        errmsg('red','bulk Richardson number is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                    ...
                    try
                        L_up_low(idx, 1) = -theta_v_low(idx, 1) .* ((u_star_low(idx, 1)).^3);
                        L_down_low(idx, 1) = k * g .* w_theta_v_low(idx, 1);
                        L_lconv_15(idx, 1) = L_up_low(idx, 1) ./ L_down_low(idx, 1);
                        Term_3_lconv_15(idx, 1) = lower_height ./ L_lconv_15(idx, 1);
                    catch
                        errmsg('red','Obukhov length for lower surface is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                    ...
                    try
                        L_up_high(idx, 1) = -theta_v_high(idx, 1) .* ((u_star_high(idx, 1)).^3);
                        L_down_high(idx, 1) = k * g .* w_theta_v_high(idx, 1);
                        L_lconv_20(idx, 1) = L_up_high(idx, 1) ./ L_down_high(idx, 1);
                        Term_3_lconv_20(idx, 1) = upper_height ./ L_lconv_20(idx, 1);
                    catch
                        errmsg('red','Obukhov length for higher surface is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                ...
                % Form a csv dump
                tempT_lconv_bulk2 = table(CDT_Time, bar_fence, R_Bulk_lconv_15_20, L_lconv_15, L_lconv_20, Term_3_lconv_15, Term_3_lconv_20);
                table_name_lconv_bulk2 = strcat(targetDate, '_calculation_lconv_15.0-20.0.csv');
                writetable(tempT_lconv_bulk2, table_name_lconv_bulk2);
                
                % Dump for calculation
                tempT_uconv_dump = table(CDT_Time, bar_fence, theta_v_low, theta_v_high, w_theta_v_low, w_theta_v_high, u_lower, u_upper, v_lower, v_upper, u_w_prime_low, u_w_prime_high, v_w_prime_low, v_w_prime_high);
                table_name_uconv_dump = strcat(targetDate, '_dump_lconv_15.0-20.0.csv');
                writetable(tempT_uconv_dump, table_name_uconv_dump);
            ...
            elseif iteration == 21
                ...
                lower_height = 6.0;
                %             delta_theta_v = temperatureTable{:, 19} - temperatureTable{:, 18};
                delta_theta_v = windTable{:, 163} - windTable{:, 162};        % tc_high_tower - tc_low_tower
                delta_z = upper_height - lower_height;
                %             theta_v = temperatureTable{:, 18};
                theta_v_low = windTable{:, 162} + 273;           % tc_low_tower + 273
                theta_v_high = windTable{:, 163} + 273;          % tc_high_tower + 273
                delta_u = windTable{:, 118} - windTable{:, 117};    % u_high_tower - u_low_tower
                delta_v = windTable{:, 133} - windTable{:, 132};  % v_high_tower - v_low_height
                u_star_low = (((windTable{:, 192}).*(windTable{:, 192})) + ((windTable{:, 207}).*(windTable{:, 207}))).^(0.25);     % u_w__low_tower * u_w__low_tower   + v_w__low_tower * v_w__low_tower
                u_star_high = (((windTable{:, 193}).*(windTable{:, 193})) + ((windTable{:, 208}).*(windTable{:, 208}))).^(0.25);    % u_w__high_tower * u_w__high_tower + v_w__high_tower * v_w__high_tower
                w_theta_v_low = windTable{:, 177};               % w_tc__low_tower
                w_theta_v_high = windTable{:, 178};              % w_tc__high_tower
                ...
                for idx = 1:length(CDT_Time)
                    try
                        R_Bulk_up(idx, 1) = g .* delta_theta_v(idx, 1) * delta_z;
                        R_Bulk_down(idx, 1) = theta_v_low(idx, 1) .* ((delta_u(idx, 1).*delta_u(idx, 1)) + (delta_v(idx, 1).*delta_v(idx, 1)));
                        R_Bulk_Rel_6_8_5(idx, 1) = R_Bulk_up(idx, 1) ./ R_Bulk_down(idx, 1);
                    catch
                        errmsg('red','bulk Richardson number is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                    ...
                    try
                        L_up_low(idx, 1) = -theta_v_low(idx, 1) .* ((u_star_low(idx, 1)).^3);
                        L_down_low(idx, 1) = k * g .* w_theta_v_low(idx, 1);
                        L_Rel_6(idx, 1) = L_up_low(idx, 1) ./ L_down_low(idx, 1);
                        Term_3_Rel_6(idx, 1) = lower_height ./ L_Rel_6(idx, 1);
                    catch
                        errmsg('red','Obukhov length for lower surface is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                    ...
                    try
                        L_up_high(idx, 1) = -theta_v_high(idx, 1) .* ((u_star_high(idx, 1)).^3);
                        L_down_high(idx, 1) = k * g .* w_theta_v_high(idx, 1);
                        L_Rel_8_5(idx, 1) = L_up_high(idx, 1) ./ L_down_high(idx, 1);
                        Term_3_Rel_8_5(idx, 1) = upper_height ./ L_Rel_8_5(idx, 1);
                    catch
                        errmsg('red','Obukhov length for higher surface is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                ...
                % Form a csv dump
                tempT_uconv_bulk = table(CDT_Time, bar_fence, R_Bulk_Rel_6_8_5, L_Rel_6, L_Rel_8_5, Term_3_Rel_6, Term_3_Rel_8_5);
                table_name_uconv_bulk = strcat(targetDate, '_calculation_rel_6.0-8.5.csv');
                writetable(tempT_uconv_bulk, table_name_uconv_bulk);
            ...
            elseif iteration == 22
                ...
                lower_height = 8.5;
                %             delta_theta_v = temperatureTable{:, 19} - temperatureTable{:, 18};
                delta_theta_v = windTable{:, 164} - windTable{:, 163};        % tc_high_tower - tc_low_tower
                delta_z = upper_height - lower_height;
                %             theta_v = temperatureTable{:, 18};
                theta_v_low = windTable{:, 163} + 273;           % tc_low_tower + 273
                theta_v_high = windTable{:, 164} + 273;          % tc_high_tower + 273
                delta_u = windTable{:, 119} - windTable{:, 118};    % u_high_tower - u_low_tower
                delta_v = windTable{:, 134} - windTable{:, 133};  % v_high_tower - v_low_height
                u_star_low = (((windTable{:, 193}).*(windTable{:, 193})) + ((windTable{:, 208}).*(windTable{:, 208}))).^(0.25);     % u_w__low_tower * u_w__low_tower   + v_w__low_tower * v_w__low_tower
                u_star_high = (((windTable{:, 194}).*(windTable{:, 194})) + ((windTable{:, 209}).*(windTable{:, 209}))).^(0.25);    % u_w__high_tower * u_w__high_tower + v_w__high_tower * v_w__high_tower
                w_theta_v_low = windTable{:, 178};               % w_tc__low_tower
                w_theta_v_high = windTable{:, 179};              % w_tc__high_tower
                ...
                for idx = 1:length(CDT_Time)
                    try
                        R_Bulk_up(idx, 1) = g .* delta_theta_v(idx, 1) * delta_z;
                        R_Bulk_down(idx, 1) = theta_v_low(idx, 1) .* ((delta_u(idx, 1).*delta_u(idx, 1)) + (delta_v(idx, 1).*delta_v(idx, 1)));
                        R_Bulk_Rel_8_5_10(idx, 1) = R_Bulk_up(idx, 1) ./ R_Bulk_down(idx, 1);
                    catch
                        errmsg('red','bulk Richardson number is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                    ...
                    try
                        L_up_low(idx, 1) = -theta_v_low(idx, 1) .* ((u_star_low(idx, 1)).^3);
                        L_down_low(idx, 1) = k * g .* w_theta_v_low(idx, 1);
                        L_Rel_8_5(idx, 1) = L_up_low(idx, 1) ./ L_down_low(idx, 1);
                        Term_3_Rel_8_5(idx, 1) = lower_height ./ L_Rel_8_5(idx, 1);
                    catch
                        errmsg('red','Obukhov length for lower surface is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                    ...
                    try
                        L_up_high(idx, 1) = -theta_v_high(idx, 1) .* ((u_star_high(idx, 1)).^3);
                        L_down_high(idx, 1) = k * g .* w_theta_v_high(idx, 1);
                        L_Rel_10(idx, 1) = L_up_high(idx, 1) ./ L_down_high(idx, 1);
                        Term_3_Rel_10(idx, 1) = upper_height ./ L_Rel_10(idx, 1);
                    catch
                        errmsg('red','Obukhov length for higher surface is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                ...
                % Form a csv dump
                tempT_uconv_bulk = table(CDT_Time, bar_fence, R_Bulk_Rel_8_5_10, L_Rel_8_5, L_Rel_10, Term_3_Rel_8_5, Term_3_Rel_10);
                table_name_uconv_bulk = strcat(targetDate, '_calculation_rel_8.5-10.0.csv');
                writetable(tempT_uconv_bulk, table_name_uconv_bulk);
            ...
            elseif iteration == 23
                ...
                lower_height = 3.0;
                %             delta_theta_v = temperatureTable{:, 19} - temperatureTable{:, 18};
                delta_theta_v = windTable{:, 161} - windTable{:, 160};        % tc_high_tower - tc_low_tower
                delta_z = upper_height - lower_height;
                %             theta_v = temperatureTable{:, 18};
                theta_v_low = windTable{:, 160} + 273;           % tc_low_tower + 273
                theta_v_high = windTable{:, 161} + 273;          % tc_high_tower + 273
                delta_u = windTable{:, 116} - windTable{:, 115};    % u_high_tower - u_low_tower
                delta_v = windTable{:, 131} - windTable{:, 130};  % v_high_tower - v_low_height
                u_star_low = (((windTable{:, 190}).*(windTable{:, 190})) + ((windTable{:, 205}).*(windTable{:, 205}))).^(0.25);     % u_w__low_tower * u_w__low_tower   + v_w__low_tower * v_w__low_tower
                u_star_high = (((windTable{:, 191}).*(windTable{:, 191})) + ((windTable{:, 206}).*(windTable{:, 206}))).^(0.25);    % u_w__high_tower * u_w__high_tower + v_w__high_tower * v_w__high_tower
                w_theta_v_low = windTable{:, 175};               % w_tc__low_tower
                w_theta_v_high = windTable{:, 176};              % w_tc__high_tower
                ...
                for idx = 1:length(CDT_Time)
                    try
                        R_Bulk_up(idx, 1) = g .* delta_theta_v(idx, 1) * delta_z;
                        R_Bulk_down(idx, 1) = theta_v_low(idx, 1) .* ((delta_u(idx, 1).*delta_u(idx, 1)) + (delta_v(idx, 1).*delta_v(idx, 1)));
                        R_Bulk_Rel_3_4_5(idx, 1) = R_Bulk_up(idx, 1) ./ R_Bulk_down(idx, 1);
                    catch
                        errmsg('red','bulk Richardson number is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                    ...
                    try
                        L_up_low(idx, 1) = -theta_v_low(idx, 1) .* ((u_star_low(idx, 1)).^3);
                        L_down_low(idx, 1) = k * g .* w_theta_v_low(idx, 1);
                        L_Rel_3(idx, 1) = L_up_low(idx, 1) ./ L_down_low(idx, 1);
                        Term_3_Rel_3(idx, 1) = lower_height ./ L_Rel_3(idx, 1);
                    catch
                        errmsg('red','Obukhov length for lower surface is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                    ...
                    try
                        L_up_high(idx, 1) = -theta_v_high(idx, 1) .* ((u_star_high(idx, 1)).^3);
                        L_down_high(idx, 1) = k * g .* w_theta_v_high(idx, 1);
                        L_Rel_4_5(idx, 1) = L_up_high(idx, 1) ./ L_down_high(idx, 1);
                        Term_3_Rel_4_5(idx, 1) = upper_height ./ L_Rel_4_5(idx, 1);
                    catch
                        errmsg('red','Obukhov length for higher surface is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                ...
                % Form a csv dump
                tempT_uconv_bulk = table(CDT_Time, bar_fence, R_Bulk_Rel_3_4_5, L_Rel_3, L_Rel_4_5, Term_3_Rel_3, Term_3_Rel_4_5);
                table_name_uconv_bulk = strcat(targetDate, '_calculation_rel_3.0-4.5.csv');
                writetable(tempT_uconv_bulk, table_name_uconv_bulk);
            ...
            elseif iteration == 24
                ...
                lower_height = 1.5;
                %             delta_theta_v = temperatureTable{:, 19} - temperatureTable{:, 18};
                delta_theta_v = windTable{:, 160} - windTable{:, 159};        % tc_high_tower - tc_low_tower
                delta_z = upper_height - lower_height;
                %             theta_v = temperatureTable{:, 18};
                theta_v_low = windTable{:, 159} + 273;           % tc_low_tower + 273
                theta_v_high = windTable{:, 160} + 273;          % tc_high_tower + 273
                delta_u = windTable{:, 115} - windTable{:, 114};    % u_high_tower - u_low_tower
                delta_v = windTable{:, 130} - windTable{:, 129};  % v_high_tower - v_low_height
                u_star_low = (((windTable{:, 189}).*(windTable{:, 189})) + ((windTable{:, 204}).*(windTable{:, 204}))).^(0.25);     % u_w__low_tower * u_w__low_tower   + v_w__low_tower * v_w__low_tower
                u_star_high = (((windTable{:, 190}).*(windTable{:, 190})) + ((windTable{:, 205}).*(windTable{:, 205}))).^(0.25);    % u_w__high_tower * u_w__high_tower + v_w__high_tower * v_w__high_tower
                w_theta_v_low = windTable{:, 174};               % w_tc__low_tower
                w_theta_v_high = windTable{:, 175};              % w_tc__high_tower
                ...
                for idx = 1:length(CDT_Time)
                    try
                        R_Bulk_up(idx, 1) = g .* delta_theta_v(idx, 1) * delta_z;
                        R_Bulk_down(idx, 1) = theta_v_low(idx, 1) .* ((delta_u(idx, 1).*delta_u(idx, 1)) + (delta_v(idx, 1).*delta_v(idx, 1)));
                        R_Bulk_Rel_1_5_3(idx, 1) = R_Bulk_up(idx, 1) ./ R_Bulk_down(idx, 1);
                    catch
                        errmsg('red','bulk Richardson number is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                    ...
                    try
                        L_up_low(idx, 1) = -theta_v_low(idx, 1) .* ((u_star_low(idx, 1)).^3);
                        L_down_low(idx, 1) = k * g .* w_theta_v_low(idx, 1);
                        L_Rel_1_5(idx, 1) = L_up_low(idx, 1) ./ L_down_low(idx, 1);
                        Term_3_Rel_1_5(idx, 1) = lower_height ./ L_Rel_1_5(idx, 1);
                    catch
                        errmsg('red','Obukhov length for lower surface is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                    ...
                    try
                        L_up_high(idx, 1) = -theta_v_high(idx, 1) .* ((u_star_high(idx, 1)).^3);
                        L_down_high(idx, 1) = k * g .* w_theta_v_high(idx, 1);
                        L_Rel_3(idx, 1) = L_up_high(idx, 1) ./ L_down_high(idx, 1);
                        Term_3_Rel_3(idx, 1) = upper_height ./ L_Rel_3(idx, 1);
                    catch
                        errmsg('red','Obukhov length for higher surface is not calculatable @: \n');
                        errmsg('blue', '      %s\n',CDT_Time(idx, 1));
                    end
                ...
                % Form a csv dump
                tempT_uconv_bulk = table(CDT_Time, bar_fence, R_Bulk_Rel_1_5_3, L_Rel_1_5, L_Rel_3, Term_3_Rel_1_5, Term_3_Rel_3);
                table_name_uconv_bulk = strcat(targetDate, '_calculation_rel_1.5-3.0.csv');
                writetable(tempT_uconv_bulk, table_name_uconv_bulk);
